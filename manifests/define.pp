...
define hosts::define (
  ...
  $config_file_options_hash = $::hosts::config_file_options_hash,
) {
  ...
  $_config_file_path  = pick($config_file_path, "${::hosts::config_dir_path}/${name}")
  $_config_file_owner = pick($config_file_owner, $::hosts::config_file_owner)
  $_config_file_group = pick($config_file_group, $::hosts::config_file_group)
  $_config_file_mode = pick($config_file_mode, $::hosts::config_file_mode)
  $config_file_content = default_content($config_file_string, $config_file_template)

#  $_config_file_require = pick($config_file_require, $::hosts::config_file_require)
